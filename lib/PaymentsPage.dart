...
class _PaymentPageState extends State<PaymentPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      drawer: Drawer( // Adding the Drawer widget
        child: ListView(
        ...
        child: Column(
          children: [
            DataTable(
              ...
              columns: [
                DataColumn(label: SizedBox(width: 100, child: Center(child: Text('Send Date')))),
                DataColumn(label: SizedBox(width: 100, child: Center(child: Text('Receive Date')))),
                DataColumn(label: SizedBox(width: 120, child: Center(child: Text('Sender Name')))),
                DataColumn(label: SizedBox(width: 120, child: Center(child: Text('Payment Method')))),
                DataColumn(label: SizedBox(width: 80, child: Center(child: Text('Price')))),
              ],
              rows: showAllRows ? rows : rows.sublist(0, 2), // Show all rows if showAllRows is true, else show only the first two rows
            ),
            ElevatedButton(
              onPressed: () {
                setState(() {
              ...
              child: Text(showAllRows ? 'Show Less' : 'Show More'), // Change button text based on showAllRows state
            ),
          ],
        ),
      ),
